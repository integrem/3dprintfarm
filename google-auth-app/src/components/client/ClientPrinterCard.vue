    ...
    <div class="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
        ...
        <div class="space-y-2">
            <div class="flex items-center text-sm text-gray-600">
                ...
                <span>{{ printer.location?.city }}{{ printer.location?.suburb ? `, ${printer.location.suburb}` : ''
                    }}</span>
            </div>
        ...
        <div v-if="printer.capabilities?.length" class="mt-4">
            ...
            <div class="flex flex-wrap gap-2">
                <span v-for="capability in printer.capabilities" :key="capability"
        ...
        <div class="mt-6">
            <button @click="$emit('select', printer)" :disabled="printer.status !== 'online'"
                ...
                {{ printer.status === 'online' ? 'Submit Print Job' : 'Unavailable' }}
            </button>
        </div>
    </div>
</template>
